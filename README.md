...
### 2. social pressure
Similarly, there is a national-level monthly data file named `aggregate_time_series.csv` and a code file named `ols analysis.R` in this subfolder. However, we introduce low-diversity index (interpreted cautiously as a proxy for social pressure) to replace those four diversity indicies. And the results obtained by the code file corresponds to model (6) in Table 3.
